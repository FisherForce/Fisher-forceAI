...
// === HISTORIQUE DES CONSEILS IA (stockage + affichage) ===
function saveAdviceToHistory(adviceData) {
  let history = JSON.parse(localStorage.getItem('fisherAdviceHistory') || '[]');
  
  const entry = {
    date: new Date().toLocaleString('fr-FR'),
    species: adviceData.species || "Inconnu",
    conditions: adviceData.conditions || "Non spécifié",
    spotType: adviceData.spotType || "Non spécifié",
    structure: adviceData.structure || "Non spécifié",
    temperature: adviceData.temperature !== null ? adviceData.temperature + "°C" : "Non spécifié",
    lures: adviceData.lures || ["Aucun conseil précis"],
    score: adviceData.score || null
  };
  
  history.unshift(entry); // le plus récent en haut
  if (history.length > 50) history = history.slice(0, 50); // limite à 50
  
  localStorage.setItem('fisherAdviceHistory', JSON.stringify(history));
}

function showAdviceHistory() {
  const list = document.getElementById('adviceHistoryList');
  const history = JSON.parse(localStorage.getItem('fisherAdviceHistory') || '[]');
  
  if (history.length === 0) {
    list.innerHTML = `<p style="color:#888;text-align:center;">Aucun conseil enregistré pour l'instant</p>`;
  } else {
    list.innerHTML = history.map(entry => `
      <div style="background:#003366;padding:18px;border-radius:15px;margin:15px 0;">
        <p style="color:#00d4aa;font-size:14px;margin:0;">${entry.date}</p>
        <p style="font-size:18px;margin:8px 0;"><strong>${entry.species}</strong> – ${entry.spotType}</p>
        <p style="color:#aaa;font-size:16px;margin:5px 0;">
          ${entry.temperature} • ${entry.conditions} • Structure : ${entry.structure}
        </p>
        <div style="margin-top:10px;">
          <p style="color:#00ff9d;font-weight:bold;">Conseils :</p>
          <ul style="margin:5px 0;padding-left:20px;">
            ${entry.lures.map(l => `<li style="color:#00ff9d;">${l}</li>`).join('')}
          </ul>
        </div>
        ${entry.score !== null ? `<p style="color:#ffd700;font-weight:bold;margin-top:10px;">Score spot : ${entry.score}/100</p>` : ''}
      </div>
    `).join('');
  }
...
// Appelle cette fonction à chaque fois que tu donnes un conseil
// Exemple dans ton fetch /api/advice après avoir reçu conseil :
saveAdviceToHistory({
  species: species,
  conditions: conditionsText,
  spotType: spotType,
